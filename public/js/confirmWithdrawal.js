...
window.addEventListener('load', function () {
  ...
  confirmBtn.addEventListener('click', async (e) => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/withdraw/${urlID}`, {
        method: 'PATCH',
        headers: {
      ...
      if(response.status === 200){
        statusInput.value = ''
        confirmBtn.textContent = 'Withdrawal Confirmed'
        alertDOM.classList.add('show')
        alertDOM.textContent = `Withdrawal has been confirmed successfully`
        setInterval(() => {
